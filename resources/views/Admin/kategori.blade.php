...
<body>
    ...
    <div class="p-4 space-y-4 sm:ml-64">
        ...
        <div x-data="{ open: false }">
            ...
            <div class="flex space-x-2 items-center hover:text-indigo-600 hover:fill-indigo-600 cursor-pointer" @click="open = true">
                ...
                <p class="font-medium text-md">Tambah Kategori</p>
            </div>
            <!-- Modal -->
            <div x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
                <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
                    <h3 class="font-medium pb-2">Tambah Kategori</h3>
                    <form action="tambahkategori" method="POST" class="space-y-2">
                        @csrf
                        <div>
        ...
        <table class="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
                ...
                <tr class="odd:bg-gray-200 even:bg-gray-300 border-gray-700 text-black">
                    ...
                    <td class="px-6 py-3">{{ $k->nama_kategori }}</td>
                    <td class="px-6 py-3">{{ $k->deskripsi }}</td>
                    <td class="px-6 py-3 space-x-4">
                        <button 
                            class="font-medium text-blue-500 hover:underline" 
                            data-modal-target="crud-modal-{{$k->id}}" 
                            data-modal-toggle="crud-modal-{{$k->id}}">
                            Edit
                        </button>
                    </td>
                </tr>
    ...
    @foreach($kategoris as $k)
        <div id="crud-modal-{{$k->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow p-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" 
                                ...
                                data-modal-hide="crud-modal-{{$k->id}}">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                    ...
                    <form action="{{ route('Admin.updatekategori', $k->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <div class="space-y-2">
                            <div>
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" 
                                        id="deskripsi" 
                                        class="mt-2 w-full rounded-md border-0 p-4 text-gray-900 shadow-sm ring-1 ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm">{{ $k->deskripsi }}</textarea>
                            </div>
                        </div>
                        <!-- Modal footer -->
                        <div class="flex justify-end space-x-2">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">   
        <div class="relative bg-white rounded-lg shadow-lg max-w-md w-full m-4">
            <div class="p-6">
                ...
                <p class="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus data kategori yang dipilih? <span class="text-red-500">Menghapus data kategori ini akan menghapus juga data buku dengan kategori yang sama !</span></p>
                <div class="flex justify-end space-x-4">
                    <button id="cancelDelete" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300">
                        Batal
                    </button>
                    <form id="deleteForm" action="{{ route('deletekategori') }}" method="POST">
                        @csrf
                        @method('DELETE')
