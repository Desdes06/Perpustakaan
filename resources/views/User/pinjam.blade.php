            ...
            <div class="grid grid-cols-7 gap-12 max-sm:grid-cols-2 max-sm:gap-6 max-md:grid-cols-3 max-md:gap-5 max-lg:grid-cols-4 max-lg:gap-4">
                @foreach($pinjam as $p)       
                    <div class="bg-[#413C88]/20 hover:shadow-xl">
                        <a href="{{ route('User.detail', ['hash' => \Vinkla\Hashids\Facades\Hashids::encode($p->buku->id)]) }}">
                            <div class="h-auto">
                            ...
                            <div class="pb-2 pt-1 px-2 flex space-x-1">
                                <a href="{{ route('User.baca.buku', ['hash' => \Vinkla\Hashids\Facades\Hashids::encode($p->buku->id)]) }}" title="Baca"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // buka modal
        document.querySelectorAll('.open-kembalikan-modal').forEach(button => {
        ...
        // tutup modal
        document.querySelectorAll('.close-kembalikan-modal').forEach(button => {
            button.addEventListener('click', () => {
                const targetId = button.getAttribute('data-target');
                const modal = document.getElementById(targetId);
                if (modal) {
